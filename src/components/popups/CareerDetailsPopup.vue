<template>
    <button v-if="careerId" @click="changeCareerID(careerId)" class="btn btn-maroon" data-bs-target="#career-details" data-bs-toggle="modal">  
      Details
    </button>
...
export default {
  ...
  mounted(){
   ...
   methods: {
      async changeCareerID(newID) {
        this.careerID = newID;

        try {
          const response = await axios.get(`http://192.168.11.144:5001/api/careers/${newID}`);
